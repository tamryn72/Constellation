# Constellation — Technical Specification

> This document is the full design spec for Constellation's rendering model,
> data structures, and stitch library. For getting-started and usage docs,
> see [README.md](README.md). For the license, see [LICENSE](LICENSE).

---

## Project Overview

A browser-based visual crochet pattern designer. Two modes: flat grid and radial/round. Stitches are SVG shapes built from "legs" drawn between anchor points, so every stitch looks like the real thing and sits in proportional grid space. Built as static HTML + CSS + ES modules; no build step, no framework.
